...
considers its own moves and never the opponents. Because of this the bots
agenda can be destroyed by the opponent by blocking the path it was about
to take.

To counter this we can use the \emph{MINIMAX-algorithm}. The \emph{MINIMAX-algorithm} tries to \emph{minimize} the board-evaluation function for itself while trying to maximize the board-evaluation function of its opponent.
...
\Else
  ...
  \State best = $-\infty$
  \ForAll{move $\in$ possible-moves(board, opp-id)}
    \State val = -MINIMAX-VALUE(update-board(board, move), \\
